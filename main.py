...
if __name__ == "__main__":
  ...
  if args.task == 1:
    ...
    # Task 4: Trajectory Optimization
        ...
        print(">> Initializing Task 4: Optimized Manipulation Pipeline")
        start_clock = time.time()
        
        # Run calculation in headless mode first
        workflow_data = opt.smart_manipulation_routine()
        
        total_time = time.time() - start_clock
        print(f">> Computation Finished: {total_time:.2f} seconds")
        print(f">> Trajectory Density: {len(workflow_data['approach_cmds'])} approach steps, "
              f"{len(workflow_data['deliver_cmds'])} delivery steps")

        if workflow_data["success"]:
            print(">> Solution Verified. Commencing GUI Playback...")
            
            # Reset the visual environment
            pgui.resetSimulation() 
            active_sim = sim.PandaSim(pgui)
            utils.setup_env(active_sim)
            config_pdef = setup_pdef(active_sim)
            
            # Reset to start and settle physics
            active_sim.restore_state(config_pdef.get_start_state())
            for _ in range(5): active_sim.step()
            
            # Playback Approach
            for cmd in workflow_data["approach_cmds"]:
                active_sim.execute(cmd, sleep_time=0.04)
            
            # Physical Interaction
            active_sim.grasp()
            opt.bind_object_to_hand(active_sim)
            
            # Playback Delivery
            for cmd in workflow_data["deliver_cmds"]:
                active_sim.execute(cmd, sleep_time=0.04)
            
            print(">> Sequence Complete. Idle mode active.")
            while True: pass
        else:
