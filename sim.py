...
class PandaSim(object):
  ...
  def execute(self, ctrl, sleep_time=0.0):
    ...
    for i in range(n_steps):
      ...
      ########## TODO ##########
      # 1. Get current joint angles for the 7-DoF robot arm
      joint_states = self.bullet_client.getJointStates(self.panda, range(7))
      q = np.array([state[0] for state in joint_states])

      # 2. Numerically evaluate the Jacobian matrix [cite: 31, 69]
      J = self.jac_solver.get_jacobian_matrix(q)

      # 3. Calculate projected joint velocities using the pseudoinverse
